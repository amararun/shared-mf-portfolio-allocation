...
// Import necessary functions from main.js - we need to modify this to use a different approach
// since circular imports can cause issues

// Define placeholder functions that will be replaced when main.js loads
let compareAndHighlightSchemas = (a, b) => console.log('Schema comparison function not yet initialized');
let compareAndHighlightTNAV = (a, b) => console.log('TNAV comparison function not yet initialized');

// This function will be called from main.js to set the actual implementations
export function setCompareFunctions(schemaFn, tnavFn) {
    console.log('Setting compare functions in api-processor.js');
    compareAndHighlightSchemas = schemaFn;
    compareAndHighlightTNAV = tnavFn;
}
                    ...
                    json_schema: {
                        ...
                        schema: {
                            ...
                            properties: {
                                dataStartRow: {
                                    type: "number",
                                    description: "The row number where actual data starts (excluding headers)"
                                },
                                columns: {
                                    type: "object",
                                    properties: {
                                        isin: {
                                            type: "string",
                                            description: "Column letter (A-Z) for ISIN"
                                        },
                                        instrumentName: {
                                            type: "string",
                                            description: "Column letter (A-Z) for Instrument Name"
                                        },
                                        marketValue: {
                                            type: "string",
                                            description: "Column letter (A-Z) for Market Value"
                                        },
                                        quantity: {
                                            type: "string",
                                            description: "Column letter (A-Z) for Quantity"
                                        }
                                    },
                                    required: ["isin", "instrumentName", "marketValue", "quantity"],
                                    additionalProperties: false
                                }
                            },
                            required: ["dataStartRow", "columns"],
                            additionalProperties: false
                        }
                    }
                }
...
async function analyzeWithAI(initialRows, file, sheetName) {
    try {
        ...
        const apiCalls = [
            // OpenAI API call for structure (now via OpenRouter)
            (async () => {
                // Use the helper function to get the OpenRouter model name
                const modelName = CONFIG.getOpenRouterModelName(CONFIG.PRIMARY_MODEL_STRUCTURE);
                ...
                logToConsole('\n=== OpenAI Structure Analysis Request (via OpenRouter) ===');
                logToConsole('URL:', `${CONFIG.RT_ENDPOINT}/open-router-completion`);
                logToConsole('Model:', modelName);
            ...
            // OpenAI API call for total value (now via OpenRouter)
            (async () => {
                // Use the helper function to get the OpenRouter model name
                const modelName = CONFIG.getOpenRouterModelName(CONFIG.PRIMARY_MODEL_MARKET_VALUE);
                ...
                logToConsole('\n=== OpenAI Total Value Analysis Request (via OpenRouter) ===');
                logToConsole('URL:', `${CONFIG.RT_ENDPOINT}/open-router-completion`);
                logToConsole('Model:', modelName);
                logToConsole('Request Body:');
                logToConsole(JSON.stringify(requestBody, null, 2));
                                ...
                                json_schema: {
                                    ...
                                    schema: {
                                        ...
                                        properties: {
                                            totalValue: {
                                                type: "number",
                                                description: "Total market value or NAV of the portfolio"
                                            }
                                        },
                                        required: ["totalValue"],
                                        additionalProperties: false
...
export { analyzeWithGemini, analyzeWithAI }; 